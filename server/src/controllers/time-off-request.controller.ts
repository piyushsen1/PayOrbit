...
import { TimeOffRequestStatus } from '../entities/TimeOffRequest';
...
export async function listRequestsHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const status = typeof req.query.status === 'string' ? (req.query.status as TimeOffRequestStatus) : undefined;
    const search = typeof req.query.search === 'string' ? req.query.search : undefined;
    const pagination = parsePagination(req.query as { page?: number; limit?: number });
    const { items, meta } = await requestService.listRequests({ employeeId, status, search }, pagination);
